...
function onBidiConnection(ws, req) {
    try {
        ...
        const { bidiHandlerDriver, proxyClient, send, sendToProxy, logSocketErr } = initBiDiSocketFunc(ws, req);
        const initBidiSocketHandlersFunc = initBidiSocketHandlers
            .bind(this);
        initBidiSocketHandlersFunc(ws, proxyClient, send, sendToProxy, bidiHandlerDriver, logSocketErr);
        if (proxyClient) {
            const initBidiProxyHandlersFunc = initBidiProxyHandlers
                .bind(bidiHandlerDriver);
            initBidiProxyHandlersFunc(proxyClient, ws, send);
        }
        const initBidiEventListenersFunc = initBidiEventListeners
            .bind(this);
 ...
 * @param driver
 */
async function onBidiMessage(data, driver) {
    let resMessage;
    let id = 0;
    const driverLog = driver.log;
    const dataTruncated = lodash_1.default.truncate(data.toString(), { length: MAX_LOGGED_DATA_LENGTH });
    try {
        let method;
        let params;
        try {
        ...
        resMessage = lodash_1.default.has(err, 'bidiErrObject')
...
function initBidiProxyHandlers(proxyClient, ws, send) {
    ...
    proxyClient.on('close', (code, reason) => {
        ...
        const intCode = lodash_1.default.isNumber(code) ? code : parseInt(code, 10);
        if (lodash_1.default.isNaN(intCode) || intCode < MIN_WS_CODE_VAL || intCode > MAX_WS_CODE_VAL) {
 ...
 */
function initBidiSocketHandlers(ws, proxyClient, send, sendToProxy, bidiHandlerDriver, logSocketErr) {
    const driverLog = bidiHandlerDriver.log;
    ...
    ws.on('close', (code, reason) => {
        ...
        if (!lodash_1.default.isEmpty(eventLogCounts)) {
 ...
 * client
 */
function initBidiEventListeners(ws, bidiHandlerDriver, send) {
    ...
    const eventListener = async ({ context, method, params = {} }) => {
        ...
        if (!method || !params) {
            driverLog.warn(`Driver emitted a bidi event that was malformed. Require method and params keys ` +
                `(with optional context). But instead received: ${lodash_1.default.truncate(JSON.stringify({
                    context,
                    method,
                    params,
        ...
        if (ws.readyState !== ws_1.default.OPEN) {
            ...
            if (ws.readyState > ws_1.default.OPEN) {
                ...
                // leaks
                driverEe.removeListener(constants_1.BIDI_EVENT_NAME, eventListener);
            }
            return;
        }
        const eventSubs = bidiHandlerDriver.bidiEventSubs[method];
        if (lodash_1.default.isArray(eventSubs) && eventSubs.includes(context)) {
            if (method in eventLogCounts) {
                ++eventLogCounts[method];
            }
            else {
                driverLog.info(`<-- BIDI EVENT ${method} (context: '${context}', ` +
                    `params: ${lodash_1.default.truncate(JSON.stringify(params), { length: MAX_LOGGED_DATA_LENGTH })}). ` +
    ...
    driverEe.on(constants_1.BIDI_EVENT_NAME, eventListener);
}
